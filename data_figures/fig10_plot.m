function fig10_plot
    cutsRounds = 20;
    Abounds = csvread('fig10-11_data.csv', 3,0,[3, 0, 3+cutsRounds ,13])*100;
    As = csvread('fig10-11_data.csv', 26,0,[26, 0, 26+cutsRounds ,13])*100;
    Av = csvread('fig10-11_data.csv', 49,0,[49, 0, 49+cutsRounds-1 ,13])*100;
    set(gcf, 'renderer','OpenGL');
    rdnText = 'Random sel.';
    NNtext = 'Optimality sel. (estimated $\hat{\mathcal{I}}_X(\rho)>0$)';
    EXtext = 'Optimality sel. (exact $\mathcal{I}_X(\rho)>0$)';
    Feastext = 'Feasibility sel.';
    Combtext = 'Combined sel.';
    fontSizeLatex =10;
    figW = 375;
    figH = 275;
    ...
    set(plot1(1),'DisplayName','Random selection');
    plot(Av(:,14),'DisplayName','Optimality selection based on exact $\mathcal{I}_X(\rho)$, where only $\rho$ with $\mathcal{I}_X(\rho)>0$ are considered for valid cuts','LineWidth',8,'LineStyle',':','Color',[0.301960784313725 0.749019607843137 0.929411764705882]);
    plot(Av(:,2),'DisplayName','Optimality selection based on estimated $\hat{\mathcal{I}}_X(\rho)$, where only $\rho$ with $\hat{\mathcal{I}}_X(\rho)>0$ are considered for valid cuts','LineWidth',3,'Color',[0 0 1],'LineStyle','-','Marker','o', 'MarkerSize',10);
    plot(Av(:,3),'DisplayName','Feasibility selection based on $\lambda_{\min}(\rho)$ finding most violated (valid) cuts','MarkerSize',10,'Marker','square','LineWidth',3,'Color',[1 0 0],'LineStyle','-');
    plot(Av(:,1),'DisplayName','Combined (optimality + feasibility) selection based on $\mathcal{C}(\rho)$','LineWidth',4,'Color',[0 0 0],'LineStyle','-');
    xlim(axes1,[1 20]);
    box(axes1,'on');
    set(axes1,'FontSize',10,'XGrid','on');
    xlabel('Cut rounds','Interpreter','latex','FontSize',fontSizeLatex+4);
    set(gcf,'color','w');
    saveas(gcf,'fig10_resV1.fig');
    print(gcf,'fig10_resV1','-dpng','-r300');
    
    % save common legend
    h = findobj(gca,'Type','line');
    legend1 = legend([h(1),h(3),h(2),h(4),h(14)]);
    ...
    xlabel('Cut rounds','Interpreter','latex','FontSize',fontSizeLatex+4);
    set(gcf,'color','w');
    saveas(gcf,'fig10_resS1.fig');
    print(gcf,'fig10_resS1','-dpng','-r300');
    close(gcf);
    ...
    set(gcf,'color','w');
    saveas(gcf,'fig10_resV2.fig');
    print(gcf,'fig10_resV2','-dpng','-r300');
    close(gcf);
    ...
    xlabel('\% of $\mathcal{M}$ to $(\mathcal{M}+\mathcal{S})$ gap closed','Interpreter','latex','FontSize',fontSizeLatex+4);
    set(gcf,'color','w');
    saveas(gcf,'fig10_resS2.fig');
    print(gcf,'fig10_resS2','-dpng','-r300');
    close(gcf);
end
...
function saveLegendToImage(figHandle, legHandle)
    ...
    %save legend
    saveas(figHandle,'fig10_legend.fig');
    print(figHandle,'fig10_legend','-dpng','-r300');
    warning('on','all');
    close(gcf);
end
